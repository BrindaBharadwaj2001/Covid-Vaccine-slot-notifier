...
def contact_us():
    if (request.method == "POST"):
        ...
        message = Message(name,sender=email,recipients=['dpbp2022@gmail.com'])
    ...
    if form.validate_on_submit():
        ...
        try:
            with sql.connect(currentdirectory + "\coviddatabase.db") as con:
                ...
                con.commit()#commits the current transaction.It will not auto commit...so,it is imp to call this each and every time
            receiver = email
...
def list():
    ...
    for row in rows:
        ...
        flag=0
        if result.ok:
            response_json = result.json()#creating the response object
            ...
            if flag == 1:
                receiver = email
                msg = Message('Vaccine Notifier', sender='darshuvs13@gmail.com', recipients=[receiver])
                msg.body = s
                mail.send(msg)
